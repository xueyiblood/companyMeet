package com.yuanch.common.aspectj;

import com.alibaba.fastjson.JSON;
import com.yuanch.common.annotation.Log;
import com.yuanch.common.enums.BusinessStatus;
import com.yuanch.common.enums.HttpMethod;
...
public class LogAspect {
    ...
    // 配置织入点
    @Pointcut("@annotation(com.yuanch.common.annotation.Log)")
    ...
    protected void handleLog(final JoinPoint joinPoint, final Exception e, Object jsonResult) {
        try {
            ...
            // 获取当前的用户
            LoginUser loginUser = SpringBeanUtils.getBean(TokenService.class).getLoginUser(ServletUtils.getRequest());
...
//            String ip = IpUtils.getIpAddr(ServletUtils.getRequest());
            ...
            // 保存数据库
            ScheduledManager.me().execute(ScheduledFactory.recordOper(operLog));
        } catch (Exception exp) {
            // 记录本地异常日志
            log.error("==前置通知异常==");
    ...
    private void setRequestValue(JoinPoint joinPoint, SysOperLog operLog) throws Exception {
        ...
        if (HttpMethod.PUT.name().equals(requestMethod) || HttpMethod.POST.name().equals(requestMethod)) {
            ...
            String params = ServletUtils.getRequest().getQueryString();
            operLog.setOperParam(StringUtils.substring(params, 0, 2000));
        } else {
